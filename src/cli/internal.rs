...
async fn stream_ws_multi(
    ...
    let joiner = async {
        ...
        while let Some(res) = tasks.next().await {
            match res {
                Err(err) => {
                    failures += 1;
                    log!("log task ended unexpectedly: {err}");
                }
                Ok(Err(err)) => {
                    failures += 1;
                    log!("log stream error: {err}");
                }
                Ok(Ok(())) => {}
            }
        }
    ...
    let all_failed = tokio::select! {
        _ = signal::ctrl_c() => { let _ = tx.send(()); return Ok(()); }
        failures = joiner => failures == total,
    };

    if all_failed {
        anyhow::bail!("All log streams failed")
    } else {
        Ok(())
    }
}
